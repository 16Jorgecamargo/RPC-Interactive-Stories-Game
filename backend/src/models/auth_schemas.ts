...
export const RegisterSchema = z.object({
  ...
  confirmPassword: z.string().min(6).openapi({ example: 'senha123', description: 'Confirmação de senha' }),
});
...
export const UserSchema = z.object({
  ...
  createdAt: z.string().openapi({ example: '2025-10-01T12:00:00Z', description: 'Data de criação' })
});
...
export const RegisterResponseSchema = z.object({
  ...
  userId: z.string().optional().openapi({ example: 'user_123abc', description: 'ID do usuário criado' }),
  message: z.string().optional().openapi({ example: 'Usuário já existe', description: 'Mensagem de erro' })
});
